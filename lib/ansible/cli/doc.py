...
class RoleMixin(object):
    ...
    def _build_doc(self, role, path, collection, argspec, entry_point=None):
        """Build documentation for role entry points.

        :param role: The simple role name.
        :param path: The full path to the role or collection.
        ...
        :returns: A tuple with the FQCN role name and a doc dict, or (fqcn, None) if no entry points match.
        """
        if collection:
            fqcn = '.'.join([collection, role])
        else:
        ...
        # If we didn't add any entry points (b/c of filtering), return None for the doc.
