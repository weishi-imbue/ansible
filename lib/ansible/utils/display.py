...
def proxy_display(method):
    ...
    sends the method name and exact caller-supplied arguments across the queue
    instead of executing the method directly.

    Args:
        method (callable): the original Display class method to wrap

    Returns:
        callable: a wrapper function with signature (self, *args, **kwargs)
    """
    @wraps(method)
    def proxyit(self, *args, **kwargs):
        if self._final_q:
            # If _final_q is set, that means we are in a WorkerProcess
            # and instead of executing the method directly from the fork
            # we will proxy it through the queue
...
class Display(metaclass=Singleton):
    ...
    def display(self, msg, color=None, stderr=False, screen_only=False, log_only=False, newline=True):
        ...
        # The @proxy_display decorator handles _final_q checking and proxying
