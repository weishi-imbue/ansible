...
class CryptHash(BaseHash):
    ...
    def _clean_ident(self, ident):
        if self.algorithm == 'bcrypt':
            # Valid BCrypt idents are '2', '2a', '2y', '2b'
            valid_idents = ['2', '2a', '2y', '2b']
            if ident is None:
                # Default to '2a' for backward compatibility
                return '2a'
            elif ident in valid_idents:
                return ident
            else:
                raise AnsibleError("BCrypt ident must be one of: %s" % ', '.join(valid_idents))
        else:
            # For non-BCrypt algorithms, ident is ignored
            return None

    def _hash(self, secret, salt, rounds, ident):
        # Use ident for BCrypt if provided, otherwise use the default crypt_id
