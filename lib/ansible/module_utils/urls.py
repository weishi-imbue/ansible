...
def prepare_multipart(fields):
    ...
    Payload content from a file will be base64 encoded by default and will
    include the appropriate ``Content-Transfer-Encoding`` and
    ``Content-Type`` headers. The encoding can be changed by setting
    ``multipart_encoding`` to a supported value (e.g. ``7or8bit``).
